...
export class Projectile {
  ...
  constructor(
    ...
    this.i = new Vector(0, 0);
    this.d = new Vector(0, 0);

    this.intStored = new Vector(0, 0);
  ...
  update() {
    ...
    if (this.target) {
      ...
      // Error
      const err = Vector.sub(this.target.pos, this.pos);
      ...
      this.intStored.setMag(
        Math.min(
          Math.max(this.intStored.mag(), -1 * this.INT_SATURATION),
          this.INT_SATURATION
        )
      );
